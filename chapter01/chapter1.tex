...
\begin{enumerate}
  ...
  \item A qubit-specific \textbf{measurement} capability. Depending on the physical implementation the measurement procedure will vary, but in any case some properties are always desirable. For instance, if the density matrix of the qubit is $
...
\paragraph{Given an unknown black box that works as a binary 1 bit function $f(x)$, how can we know if it is a constant function, or a balanced function evaluating it only once?\\}
\textcolor{white}{.}\\
...
\red{quantum chemistry, Quantum simulations and stuff, proteins...}.
