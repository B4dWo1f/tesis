...
As explained in chapter~\ref{ch:graphene} if we restrict ourselves to the low energy limit graphene is a bipartite lattice and since it is naturally at half filling, according to the Lieb's theorem~\cite{Lieb1989}, if there is an imbalance the number of atoms in one of the sublattices a zero energy state is expected to appear.\\
...
When using the whole $n=2$ set of orbitals, namely $s$, $p_x$, $p_y$, $p_z$, the general description does not change heavily, the zero energy state is no longer required to be exactly at $E=0$, but still an in-gap state appears by introducing a H adatom.
%~~~~~~~~~~~~~~~~~~~~~~~~~~ FIGURE ~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{figure}[h!]
\centering
...
\FloatBarrier
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
Since we are interested for now in the electronic properties at low energy we will only need the $p_z$ orbitals.
