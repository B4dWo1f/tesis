\documentclass[twoside,openright,fontsize=13pt,a4paper,footinclude=true,
               numbers=noenddot,headinclude=true]{scrreprt}

%~~~~ Encoding for this file
\usepackage[utf8]{inputenc}

%~~~~ classicthesis package
\usepackage[linedheaders,parts,dottedtoc,pdfspacing,
            eulerchapternumbers]{classicthesis}


%~~~~ margins
% \usepackage[lmargin=142pt,rmargin=95pt,tmargin=127pt,bmargin=123pt]{geometry}
% \usepackage[left=15in,right=1in,top=1in,bottom=1in]{geometry}
\usepackage[left=25mm,right=25mm,top=25mm,bottom=30mm,bindingoffset=5mm]{geometry}

%~~~~ Page numbering
% \deftripstyle{pgnumbottomcenter}{}{}{}{}{\pagemark{}}{}
% \pagestyle{pgnumbottomcenter}
% \renewcommand{\chapterpagestyle}{pgnumbottomcenter}

%~~~~ Normal math font
\usepackage{lmodern}

%~~~~ Math stuff
\usepackage{xcolor}
\usepackage{amsmath}
% \usepackage{mathabx}
\numberwithin{equation}{chapter}
\numberwithin{figure}{chapter}
\usepackage{amssymb,amsthm,dsfont}
\usepackage[version=4]{mhchem}
\usepackage[inline]{enumitem}
\renewcommand\labelenumi{\theenumi)}

%~~~~ citations
\usepackage[noadjust,super]{cite}

%~~~~ HyperLinks
\usepackage{hyperref}
\hypersetup{colorlinks=true,linkcolor=blue,citecolor=blue,
            pdftitle={Functionalized Bilayer Graphene For Quantum Technologies},
            pdfsubject={PhD Thesis},
            pdfkeywords={graphene, bilayer, slater-koster, bias, in-gap},
            pdfauthor={Noel Garcia}}
%            pdfproducer={Latex with hyperref, or other system},
%            pdfcreator={pdflatex, or other tool}]{hyperref}

%~~~~ Figures and Captions
\usepackage{subfig}
\usepackage{graphicx}  %[pdftex]
\setkeys{Gin}{width=\textwidth}
\usepackage{caption}
\captionsetup{font=small,width=0.85\textwidth,
              justification=justified,format=plain}
\usepackage{wrapfig}

%~~~~ Manage FloatBarriers and stuff
\usepackage{placeins}

%~~~~ Index depth
\setcounter{tocdepth}{2}  % For debugging. Later ---> \setcounter{tocdepth}{2}

%~~~~~~~~ Acronyms ~~~~~~~~
\usepackage[printonlyused]{acronym}
\usepackage{etoolbox}
%~~~~ Repeat at the beginning of each chapter
\preto\chapter\acresetall

%~~~~ Remove color of hyperlink for acronym
\makeatletter
\AtBeginDocument{\renewcommand*{\AC@hyperlink}[2]{%
    \begingroup
      \hypersetup{hidelinks}%
      \hyperlink{#1}{#2}%
    \endgroup
  }}
\makeatother

\usepackage{afterpage}

%~~~~ Empty page
\newcommand\blankpage{%
    \null
    \thispagestyle{empty}%
    \addtocounter{page}{-1}%
    \newpage}

%~~~~ Units
\usepackage{siunitx}
\sisetup{
  per-mode=fraction,
  fraction-function=\dfrac,
  inter-unit-product={}\cdot{},
  exponent-product={}\mspace{-2mu}\cdot\mspace{-2mu}{}
}
\usepackage{leftidx}  % http://ctan.org/pkg/leftidx

%~~~~~~~~~~~~~~~ COMANDOS PROPIOS ~~~~~~~~~~~~~~%
\newcommand{\mean}[1]{\langle#1\rangle}
\newcommand{\ket}[1]{|#1\rangle}
\newcommand{\bra}[1]{\langle#1|}
\newcommand{\braket}[2]{\langle#1|#2\rangle}
\newcommand{\crea}[2]{#1^{\dagger}_{#2}}
\newcommand{\des}[2]{#1_{#2}}
\newcommand{\grad}[1]{\vec{\nabla}#1}
\newcommand{\diver}[1]{\vec{\nabla}\vec{#1}}
\newcommand{\rotac}[1]{\vec{\nabla}\times#1}
\newcommand{\red}[1]{\textcolor{red}{#1}}
%\definecolor{blue}{rgb}{0.06, 0.3, 0.57}
%\definecolor{blue}{rgb}{0.09019608, 0.38039216, 0.70196078}
\definecolor{blue}{rgb}{0.12156863, 0.46666667, 0.70588235}
\newcommand{\blue}[1]{\textcolor{blue}{#1}}
\newcommand{\Tr}[1]{Tr\left(#1\right)}
\newcommand{\udaw}{\uparrow\mathrel{\mspace{-3mu}}\downarrow}
\newcommand{\duaw}{\downarrow\mathrel{\mspace{-3mu}}\uparrow}
\newcommand{\UDaw}{\Uparrow\mathrel{\mspace{-3mu}}\Downarrow}
\newcommand{\DUaw}{\Downarrow\mathrel{\mspace{-3mu}}\Uparrow}
\newcommand{\e}[1]{\mspace{-2mu}\cdot\mspace{-2mu}10^{#1}}
\newcommand{\ex}[1]{\mspace{-2mu}\times\mspace{-2mu}10^{#1}}
\newcommand{\fref}[2][{}]{\hyperref[#2]{Fig.~\ref{#2}~#1}}
\newcommand{\chref}[1]{\hyperref[#1]{Chapter~\ref{#1}}}
\renewcommand{\eqref}[2][{}]{\hyperref[#2]{eq.~\ref{#2}~#1}}
\newcommand{\ie}{i.\,e. }
\newcommand{\Ie}{I.\,e. }
\newcommand{\eg}{e.\,g. }
\newcommand{\Eg}{E.\,g. }
\newcommand{\sign}[1]{\operatorname{sgn}({#1})}
\newcommand{\onlinecite}[1]{~[\hspace{-1 ex} \nocite{#1}\citenum{#1}]}
%~~~~~~~ Shortcuts ~~~~~~~
\def\uaw{\uparrow}
\def\daw{\downarrow}
\def\Uaw{\Uparrow}
\def\Daw{\Downarrow}
%~~~~~~~ Shortcuts Notation ~~~~~~~
\def\up{\texttt{0}}
\def\down{\texttt{1}}
%~~~~~~~ Renew Commands ~~~~~~~
\renewcommand{\figurename}{\footnotesize{\textsc{Figure}}}
\renewcommand{\Im}[1]{\operatorname{\mathbb{I}m}\left[#1\right]}
\renewcommand{\Re}[1]{\operatorname{\mathbb{R}e}\left[#1\right]}
\AtBeginDocument{\renewcommand{\hbar}{\hslash}}
%\renewcommand{\thefootnote}{\fnsymbol{footnote}}
\renewcommand{\thefootnote}{\Roman{footnote}}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%



%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{document}
%~~~~~~~~ Cover ~~~~~~~~
% Missing

%~~~~~~~~ Title ~~~~~~~~
\input{title.tex}
\afterpage{\blankpage}

%%~~~~~~~~ Dedication ~~~~~~~~
%\input{covers/dedication.tex}

%~~~~~~~~ Agradecimientos ~~~~~~~~
\thispagestyle{empty}
\pagenumbering{gobble}
\input{covers/agradecimientos.tex}

%~~~~~~~~ Acronyms ~~~~~~~~
% \thispagestyle{empty}
% \afterpage{\blankpage}
\chapter*{Acronyms}
\pagenumbering{gobble}
\input{acro_list}
\clearpage

%~~~~~~~~ Index ~~~~~~~~
\thispagestyle{empty}
\afterpage{\blankpage}
\pagenumbering{gobble}
\tableofcontents
\clearpage

%~~~~~~~~ Start page counting
\pagenumbering{arabic}


%~~~~~~~~ Chapter ~~~~~~~~
\thispagestyle{empty}
\afterpage{\blankpage}
\input{introduction/intro_es.tex}
\input{introduction/intro.tex}

%~~~~~~~~ Chapter ~~~~~~~~
\input{graphene/graphene.tex}

%~~~~~~~~ Chapter ~~~~~~~~
% Graphene Bilayer
\input{graphene_bilayer/graphene_bilayer.tex}

%~~~~~~~~ Chapter ~~~~~~~~
\input{defects/adatom.tex}

%~~~~~~~~ Chapter ~~~~~~~~
\input{bilayer_defect/bilayer_defect.tex}

%~~~~~~~~ Chapter ~~~~~~~~
\input{defects/hyperfine.tex}

%%~~~~~~~~ Chapter ~~~~~~~~
%\input{artlat/artlat.tex}

%~~~~~~~~ Chapter ~~~~~~~~
\input{designer/designer.tex}




%%%~~~~~~~~ Chapter ~~~~~~~~
%%% Introduction & Quantum computing
%%\input{chapter01/chapter1.tex}

%%%~~~~~~~~ Chapter ~~~~~~~~
%%% Current experimental status
%%\input{chapter03/chapter3.tex}

%%%~~~~~~~~ Chapter ~~~~~~~~
%%% Graphene
%%\input{chapter04/chapter4.tex}
%%
%%%~~~~~~~~ Chapter ~~~~~~~~
%%% A Graphene based Quantum Computer
%%\input{chapter06/chapter6.tex}

%%~~~~~~~~ Chapter ~~~~~~~~
%% Hydrogenated Graphene
%%\input{chapter05/chapter5.tex}

%% %~~~~~~~~ Chapter ~~~~~~~~
%% % Conclusions
%% \input{chapter07/chapter7.tex}
%%
%% %~~~~~~~~ Chapter ~~~~~~~~
%% % Conclusions
%% \input{chapter08/chapter8.tex}
%%

%~~~~~~~~ Chapter ~~~~~~~~
\input{outlook/outlook.tex}
% \input{outlook/outlook_es.tex}
% ~~~~~~~~ Appendices ~~~~~~~~
\appendix
% Slater-Koster hoppings
\input{appendix/publications.tex}
% Topological stuff
\input{appendix/qsh.tex}
\input{ann/ann.tex}
\input{appendix/SK.tex}
%%% % Low energy regime
%%% \input{appendix/low_energy_regime.tex}
%%% Models
%%\input{appendix/models.tex}
%%% Cells
%%\input{appendix/cells.tex}
%%%%% Quantum Gates
%%%%\input{appendix/A.tex}
% Units
\input{appendix/units.tex}
% Effective exchange interaction
\input{appendix/blue_hamiltonian.tex}
%%%%\input{appendix/C.tex}




%~~~~~~~~ Bibliography ~~~~~~~~
\bibliographystyle{ieeetr}
\bibliography{all_bib}


%~~~~~~~~ Dedication ~~~~~~~~
% \input{covers/bye.tex}

\end{document}
