...
This mixing of the $p_z$ manifold with all the other orbitals happens at $\Gamma$ and it can be seen in both in the band structure and in the \ac{dos}
%~~~~~~~~~~~~~~~~~~~~~~~~~~ FIGURE ~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{figure}[h!]
\centering
...
Bismuth, or $h-BN$  % TODO cite
are simple examples in which the \ac{sk} description works well.\\

%
% Graphene TB
%
The required parameters, extracted both from the literature\cite{} and/or by fitting of \ac{dft} calculated bands, are (in $\si{\eV}$):
\begin{equation}
  \begin{array}{l|cccc}
...
\label{G_SK_params}
\end{equation}
where the hoppings with a hypothetical $H$ atom have been included for convenience.
...
Bismuth is a little bit trickier since an accurate description requires up to third neighbor hoppings (and spin-orbit coupling, which we will neglect for the moment). The \ac{sk} parameters, taken from~\citen{Liu1995} are:
\begin{equation}
  \begin{array}{c|cccc}
    Hopping & V_{ss\sigma} & V_{sp\sigma} & V_{pp\sigma} & V_{pp\pi} \\ \hline
...
The main effect of such a term is to open a gap at the Dirac points.

\subsection{Zeeman}
The application of an external magnetic field usually has several effects in a material. While a lot of interesting Physics is developed around the formation of Landau levels, we will restrict our study to the simpler case of the Zeeman splitting of the energy levels. This effect consists of a splitting of up ($\uaw$) and down ($\daw$) spin.

\subsection{Interlayer}
We can consider an adiabatic formation of bilayer graphene (or $n$-layer graphene, for that matter) by considering two layers of graphene infinitely apart and bringing them together. We can describe the Hamiltonian of the system at the first moment as two disconnected sectors of the same Hamiltonian, equivalent to the limit $\eta\rightarrow0$ in eq.~\eqref{Gbi}. The Hamiltonian in the one orbital basis $\mathcal{B}_1$ reads:
\begin{equation}
...
The first feature is that an electric field would deform the atomic orbitals, breaking the mirror symmetry which protects the $\pi$-manifold. The breaking of this symmetry opens an effective channel for mixing $s$ and $p_z$ orbitals.
...
\label{sec:soc}
The \ac{soc} is relativistic interaction that can be interpreted in classical terms (allowing the existence of spin in classical physics) as the interaction between the spin of the electron and the electrostatic potential of the nucleus.
