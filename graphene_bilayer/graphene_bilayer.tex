...
There are two ways to stack two graphene layers in a commensurate way. The first option is to align directly one layer on top of the other, so every atom is connected to an atom from the other layer. This is the so-called $AA$-stacking.
The other option is to have the top layer shifted by one atomic position as shown in \fref{Gbi_stack}a,c). The later stacking, commonly called Bernal stacking or $AB$-stacking, is the most common one according to both experiments and calculations\cite{Norimatsu2010,Charlier1994,Charlier1994a}.

In both stackings the overall band structure is quite similar (\fref{Gbi_stack}b,d)) since the interlayer interaction is around one order of magnitude weaker than the intralayer ones.
%~~~~~~~~~~~~~~~~~~~~~~~~~~ FIGURE ~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{figure}[h!]
\centering
...
\FloatBarrier
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
The main differences happen around the $K$ ($K'$) points. It can be shown in \fref{Gbi_stack}e). In the $K$ points the $\vec{k}$ dependence vanishes. The Hamiltonian, then, reads:
\begin{equation}
...
\begin{equation}
   ...
   \end{array}\right)
\end{equation}
This fact shows that we can expect two zero eigenvalues at the $K$ points for the $AB$-stacking, while there should be a gap in the $AA$-stacking.\\
...
When $\eta=0$ the band structure is just that of graphene, but twofold. In the $AA$-stacking every atom interacts with another atom from the other layer, it is to be expected a bonding-antibonding-like splitting for every $\vec{k}$, in fact that is exactly what happens, and vertical rigid shift of the band structure. Around the $K$ point this translates in having two cones placed at $E=\pm t'$.
In the $AB$-stacking only one of the atoms of each layer interacts via $t'$, one could think that around the $K$ point one of the cones hybridizes into a bonding-antibonding splitting and it would remain a cone with two gapped pseudo-cones at $E=\pm t'$, but the calculation does not yield exactly that, but rather a ``parabolized'' version of this scenario. The parabolic component comes from second order hoppings between the two layers.\\
