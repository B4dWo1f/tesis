...
\end{equation}
where each self-energy $\Sigma_{L/R}$ can be calculated by solving iteratively the coupled equations:
\begin{equation}
\begin{split}
  \Sigma_R(E,k_\parallel) &= V_R(k_\parallel)g_R(E,k_\parallel)V^{\dagger}_R(E,k_\parallel) \\
  g_R(E,k_\parallel) &= \frac{1}{E-H_0(k_\parallel)-\Sigma_R(E,k_\parallel)}
\end{split}
\end{equation}
and analogously for the left self energy $\Sigma_L$. Once the Green's function is calculated, the \ac{dos} is calculated as usual as:
\begin{equation}
