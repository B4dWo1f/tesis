...
There are only two ways to stack two graphene layers in a commensurate way keeping only two atoms per unit cell. The first option is to align directly one layer on top of the other, so every atom in one layer is connected to an atom from the other layer\fref{Gbi_stack}a). This is the so-called $AA$-stacking.
...
In order to understand these band structures we can consider the adiabatic formation of bilayer graphene (or $n$-layer graphene, for that matter) starting with two layers of graphene infinitely apart and bringing them together.
...
For $\eta>0$ we will restrict ourselves to analyze the Hamiltonian at the $\vec{k}=K$ point. In this point the $\vec{k}$ dependence vanishes $f(K)=0$. So the complete Hamiltonian \eqref{Gbi} reads:
...
In addition, the actual gap of the system occur not exactly at $K$($K'$) but in its vicinity (see \fref{Gbi_summary}{c)}), although the value is quite close to $\Delta$ and its dependence with $\mathcal{E}$ is the same.


% Maybe here a plot of the gap as a function of the electric field
...
The dual-gating setup can be considered as an infinite parallel plate capacitor, hence, the electric field only has $z$ component, hence the divergence of the displacement results in a derivative which can be discretized:
\begin{equation}
...
There are other interesting scenarios leading to ``edge'' states in this system, though.
...
We consider a \ac{2d} sheet of bilayer graphene and apply an electric field $\mathcal{E}>0$ for atomic positions with $x>0$ and $\mathcal{E}<0$ otherwise as depicted in \fref{fig:kink_states}a). Equivalently we can consider the following Hamiltonian term (in the basis \eqref{basis_bi}):
%~~~~~~~~~~~~~~~~~~~~~~~~~~ FIGURE ~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{figure}[h!]
\centering
...
We  can calculate this by considering a \ac{2d} system as an infinite collection of \ac{1d} systems coupled by some hopping. Each of this \ac{1d} systems is infinite itself and it can be defined a $k$-moment along its axis: $k_\parallel$, and the hopping between these \ac{1d} systems will also be $k_\parallel$-dependent.
...
The anticipated result is clearly seen in \fref{fig:kink_states}b), where we can see the four edge states crossing the otherwise gaped spectrum.
...
This section is based on our work on the ``Van der Waals Spin Valves'' \onlinecite{Cardoso2018}.
Ever since the synthesis of graphene there has been a race to be able produce \ac{2d} materials with different properties. A major milestone has been the synthesis of $\ce{CrI_3}$, a two dimensional ferromagnetic insulating material\cite{Klein2018,Song2018,Wang2018}. %,Jiang2018,Huang2018,Jiang2018b}

% This section is a brief summary of our publication [\citen{Cardoso2018}] were we consider the proximity effect of $\ce{CrI_3}$ on bilayer graphene. In particular
We consider bilayer graphene sandwiched by two layers of $\ce{CrI_3}$ as shown in \fref{cri3}a), with the possibility of having the upper and lower $\ce{CrI_3}$ layers in a parallel (FM) or antiparallel configuration (AM).
...
Our toy model focuses on the effective Hamiltonian for the bilayer graphene, so our starting point will be the usual Hamiltonian \eqref{Gbi}. The effect of a magnetic layer in its proximity would appear in the Hamiltonian as a spin-dependent on-site energy shift.
...
The resulting bands for this Hamiltonian are shown in \fref{cri3}b) with the color representing the spin of the electrons. For the sake of readability, in the AF case, band structure of each layer has been separated and the size of the points has been made proportional to the layer polarization, showing that the top of the valence band and bottom of the conduction band are both spin and layer polarized in each of the layers.


The spin-valve function of this system becomes apparent when considering the possibility of pinning down the magnetization of one of the $\ce{CrI_3}$ layers (for instance, as shown in \fref{cri3}a), by proximizing it with another $\ce{CrI_3}$ layer) and flipping the other with an external magnetic field.
...
Analogously to previous section we can think of a scenario in which there is a domain wall between two AF regions with opposite magnetizations.
%~~~~~~~~~~~~~~~~~~~~~~~~~~ FIGURE ~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{figure}[h!]
\centering
...
\label{chern_kink}
\end{equation}
Based on \eqref{chern_kink} we can anticipate\cite{Martin2008,San-Jose2009} the emergence of one-dimensional spin-dependent chiral in-gap states in domain walls separating two antiferromagnetic domains with opposite magnetizations [\fref{cri3kink}b)]. In order to verify this, we compute the momentum resolved density of states of a domain wall along the zigzag direction as illustrated in \fref{cri3kink}b)

The domain wall is assumed to be abrupt, preserving spin collinearity. The calculation is done for a system with translational invariance along the wall direction, and embedded between two semi-infinite gaped graphene bilayer planes, using a Green’s function technique\cite{Lado2015}. Both domains are insulating, but at each valley the Chern number is opposite for a given spin direction. Thus, a domain wall along the zigzag direction, that preserves the valley, features two chiral 1D in-gap states per valley and per spin [see \fref{cri3kink}a)]. It is interesting to note that, for a given valley, the states are spin chiral and therefore back-scattering requires either spin mixing (arising from a non-collinear domain wall, for instance), or intervalley scattering. %A non-collinear domain wall might result in spin mixing.
% XXX
